...
// Draw variables
vector<vec4>* car_vertices = new vector<vec4>();
int car_vert_sz=0,car_norm_sz=0,car_face_sz=0;
vector<vec3>* car_normals=new vector<vec3>();
vector<vec4>* car_mat=new vector<vec4>();
...
void read_obj(char *filename,vector<vec4> *vertices,vector<vec3> *normals,vector<vec4> *lights,int *mat_ind,float* shinnines,int* vertices_size,int* normals_size,int* faces_size)
{
	ifstream file,file1;
	string line,mtlpath;
	...
	if(file.is_open())
	{
		...
		while ( getline (file,line))
		{
			if(line.compare(0,1,"v")==0) // if equal return 0, thats why "!"
			{
				...
				temp_vertices.push_back(vec4(temp[0],temp[1],temp[2],1));
			}
			if(line.compare(0,2,"vn")==0) // if equal return 0, thats why "!"
			{
				num_normals++;
				sscanf(line.c_str(),"v %f %f %f",&temp[0],&temp[1],&temp[2]);
				temp_normals.push_back(vec3(temp[0],temp[1],temp[2]));
			}
			if(line.compare(0,6,"usemtl")==0) // if equal return 0, thats why "!"
			{
				...
				for(int i=0;i<cont_mat;i++)
				{
					if(line.compare(7,line.size()-7,mat_names[i]) == 0)
					{
						...
						lights->push_back(vec4(materials[i][0],materials[i][1],materials[i][2],1));
						lights->push_back(vec4(materials[i][3],materials[i][4],materials[i][5],1));
						lights->push_back(vec4(materials[i][6],materials[i][7],materials[i][8],1));
			...
			if(line.compare(0,1,"f")==0) // if equal return 0, thats why "!"
			{
				...
				vertices->push_back(temp_vertices[temp1[0]-1]);
				vertices->push_back(temp_vertices[temp1[2]-1]);
				vertices->push_back(temp_vertices[temp1[4]-1]);
				normals->push_back(temp_normals[temp1[1]-1]);
				normals->push_back(temp_normals[temp1[3]-1]);
				normals->push_back(temp_normals[temp1[5]-1]);
			}
		}
		mat_ind[0]=num_mat;
...
void init(){
	...
	glBufferData( GL_ARRAY_BUFFER, car_vertices->size()*sizeof(vector<vec4>)+car_normals->size()*sizeof(vector<vec3>),  NULL, GL_STATIC_DRAW );
	glBufferSubData( GL_ARRAY_BUFFER, 0, car_vertices->size()*sizeof(vector<vec4>), car_vertices);
	glBufferSubData( GL_ARRAY_BUFFER, car_vertices->size()*sizeof(vector<vec4>), car_normals->size()*sizeof(vector<vec3>), car_normals );
	...
	glVertexAttribPointer( vNormal, 3, GL_FLOAT, GL_FALSE, 0,
		BUFFER_OFFSET(car_vertices->size()*sizeof(vector<vec4>)) );
