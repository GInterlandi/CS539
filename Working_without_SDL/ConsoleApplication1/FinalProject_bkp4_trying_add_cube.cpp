...
std::chrono::time_point<std::chrono::system_clock> roll,pitch,yaw;//  variables for camera moving with arrow keys and XYZ
...
Angel::vec4 eye = vec4(0.0, 0.0, 3.0, 1.0);
Angel::vec4 eye0 = vec4(0.0, 0.0, 3.0, 1.0);
Angel::vec4 n = Angel::normalize(vec4(0.0, 0.0, 3.0, 1.0)); // Camera vectors
mat4 bases(n,u,v,vec4(0.0,0.0,0.0,1.0));
...
void read_obj(char *filename,vec4 *vertices,vec3 *normals,vec4 *lights,int *mat_ind,float* shinnines,int* vertices_size,int* normals_size,int* faces_size)
{
	...
	if(file.is_open())
	{
		...
		repos = Translate(-(xmax-xmin)/2000,-(ymax-ymin)/2000,-(zmax-zmin)/2000);
		mat_ind[0]=num_mat;
		num_mat++;
...
void idle(void)
{
	...
	if(key_w) 
	{
		...
		eye = eye0 + car_pos*vec4(0.0,0.0,0.0,1.0);
		t_trans = end;
	}
	if(key_s) 
	{
	...
	if(key_a) 
	{
		...
		//eye =  RotateX(CAR_TURN_SPEED*elapsed_seconds.count())*eye0 + car_pos*vec4(0.0,0.0,0.0,1.0);
		////RotateX(CAR_TURN_SPEED*elapsed_seconds.count())*Translate(car_pos*vec4(0.0,0.0,0.0,-1.0)*vec4(1.0,1.0,1.0,-1.0))*eye;
		//bases = RotateX(CAR_TURN_SPEED*elapsed_seconds.count())*bases;
		//n = bases[0];
		//u = bases[1];
		//v = bases[2];
		//t_rot = end;
...
void keyboard( unsigned char key, int x, int y )
{
	...
	case 'Z': // roll counterclockwise in the xy plane
		if(!key_Z) roll = std::chrono::system_clock::now();
		key_Z=1;
		break;
	case 'z': // roll clockwise in the xy plane
		...
		break;
	case 'X': // pitch up
		if(!key_X) pitch = std::chrono::system_clock::now();
		key_X=1;
		break;
	case 'x': // pitch down
		if(!key_x) pitch = std::chrono::system_clock::now();
		...
		break;
	case 'Y': // yaw up
		if(!key_Y) yaw = std::chrono::system_clock::now();
		key_Y=1;
		break;
	case 'y': // yaw down
		if(!key_y) yaw = std::chrono::system_clock::now();
