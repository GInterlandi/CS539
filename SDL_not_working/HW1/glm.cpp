...
GLvoid glmLoadInVBO(GLMmodel* model)
{
	...
	while (group) 
	{
		...
		int sizeTotal   = sizePoints + sizeNormals+sizeof(shiftATTR); 
		...
		glBufferSubData( GL_ARRAY_BUFFER, sizePoints+sizeNormals, sizeof(shiftATTR), shiftATTR );
...
GLvoid
	glmDrawVBOInstanced(GLMmodel* model, GLuint program)
{
	static GLMmaterial* material;
	GLuint vPosition;
	GLuint vNormal;
	...
	while(group)
	{
		...
		GLMmaterial* material = &model->materials[group->material];
		// Bind the material to the shader
		glUniform4fv( glGetUniformLocation(program, "AmbientProduct"),1, material->ambient );
		glUniform4fv( glGetUniformLocation(program, "DiffuseProduct"),1,material->diffuse );
		glUniform4fv( glGetUniformLocation(program, "SpecularProduct"),1,material->specular  );	
		...
		glEnableVertexAttribArray( vNormal );
		glVertexAttribPointer( vNormal, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(offset));		
		offset = sizeof(GLfloat)*7*3*group->numtriangles;
		...
		glDrawArraysInstanced( GL_TRIANGLES, 0, 3 * group->numtriangles,NumInstances );	  
		group = group->next;
	}
}
